...
class EntityA:
    ...
    def output(self, message):
        if not self.waiting_for_ack:
            packet = Pkt(self.seq_num, self.seq_num, calculate_checksum(self.seq_num, self.seq_num, message.data), message.data)
            self.last_packet_sent = packet
            to_layer3(self, packet)
            start_timer(self, 10)
            self.waiting_for_ack = True
            # print("Entity A: Sent Package", self.seq_num)
        else:
            self.buffer.append(message)
            # print("Entity A: Dropped message: not waiting")
    ...
    def input(self, packet):
        is_corrupt = (calculate_checksum(packet.seqnum, packet.acknum, packet.payload) != packet.checksum)
        if is_corrupt:
            return

        if (1 - self.seq_num) == packet.acknum:
            # print("Ignore duplicate ack")
            pass
        elif self.seq_num != packet.acknum:
            # print("Entity A: Corrupted or ack not matching")
            pass
        elif self.waiting_for_ack and self.seq_num == packet.acknum:
            stop_timer(self)
            self.seq_num = 1 - self.seq_num
            self.waiting_for_ack = False
            if self.buffer:
                message = self.buffer.popleft()
                packet = Pkt(self.seq_num, self.seq_num, calculate_checksum(self.seq_num, self.seq_num, message.data),
                             message.data)
                self.last_packet_sent = packet
                to_layer3(self, packet)
                start_timer(self, 10)
    ...
    def timer_interrupt(self):
        if self.last_packet_sent:
            to_layer3(self, self.last_packet_sent)
            start_timer(self, 10)
...
class EntityB:
    ...
    def input(self, packet):
        ...
        if (1 - self.expected_seqnum) == packet.seqnum:
            to_layer3(self, packet)
            # print("Dup Packet Ignored")
        elif packet.seqnum == self.expected_seqnum:
            to_layer5(self, Msg(packet.payload))
            to_layer3(self, packet)
            self.expected_seqnum = 1 - self.expected_seqnum
            # print("Entity B: Sent ACK", packet.seqnum)
        else:
            # print("Dropped packet: Corrupt or seqnum not matching")
            pass
...
def calculate_checksum(seqnum, acknum, payload):
    ...
    crc = binascii.crc32(seqnum_bytes)
    crc = binascii.crc32(acknum_bytes, crc)
